...
def download_if_necessary(
    ...
    suffix: Optional[str] = None    # to be used with path=None
    ) -> Optional[str]:
    if path is not None:
...
LANG2SUBT: dict[str, FrequencyDataSpec] = {
    ...
    # TUBELEX-JA v0.2
    'ja': FrequencyDataSpec(
        filename='data/downloads/tubelex-ja-nfkc-lower.tsv.xz',
        url=(
            'https://github.com/adno/tubelex/raw/v0.2/results/'
            'tubelex-ja-nfkc-lower.tsv.xz'
            ),
        total_row=True
        )
    }
...
class FrequencyData(NamedTuple):
    ...
    def from_wiki(
        ...
        '''
        # From wikipedia-word-frequency-clean v0.1
        ...
        return FrequencyData.from_file_url(
            ...
            url=(
                f'https://github.com/adno/wikipedia-word-frequency-clean/raw/v0.1/'
                f'results/{filename}'
                ),
            total_row=True
            )
    ...
    def from_subtitles(
        ...
        '''
        Get uncased SUBTLEX/TUBELEX frequencies for any of the supported languages.
        '''
        spec = LANG2SUBT[lang]
        return FrequencyData.from_file_url(
            **spec,
